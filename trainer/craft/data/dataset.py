...
class CustomDataset2(CraftBaseDataset):
    def __init__(
        self,
        output_size,
        data_dir,
    ...
    def load_data(self, index):
        ...
        for i in range(len(word_bboxes)):
            ...
            word_level_char_bbox.append(word_bboxes[i].reshape(-1,4,2))
    ...
    def make_gt_score(self, index):
        ...
        if len(word_level_char_bbox) == 0:
            ...
            region_score = self.gaussian_builder.generate_region(
                img_h, img_w, word_level_char_bbox, horizontal_text_bools=[True for _ in range(len(word_level_char_bbox))]
            )
            affinity_score, all_affinity_bbox = self.gaussian_builder.generate_affinity(
                img_h, img_w, word_level_char_bbox, horizontal_text_bools=[True for _ in range(len(word_level_char_bbox))]
            )
